    ...
    public static class Controller
    {
        ...
        public static void DistributeWorkers()
        {
            ...
            if (idleWorkers.Count > 0)
            {
                ...
                var gasBuildings = GetUnits(Units.GasGeysersStructures, onlyCompleted: true);

                foreach (var rc in resourceCenters)
                {
                    //get one of the closer mineral fields
                    var mf = GetFirstInRange(rc.position, mineralFields, resourceRange);
                    var gb = GetFirstInRange(rc.position, gasBuildings, resourceRange);

                    if (mf == null && gb == null) continue;

                    //only one at a time
                    Logger.Info("Distributing idle worker: {0}", idleWorkers[0].tag);
                    if (random.Next(2) == 0 && mf != null)
                    {
                        idleWorkers[0].Smart(mf);
                    }
                    else if (gb != null)
                    {
                        idleWorkers[0].Smart(gb);
                    }
                    return;
                }
                //nothing to be done
            ...
            else
            {
                ...
                if ((transferFrom != null) && (transferTo != null))
                {
                    ...
                    foreach (var worker in workers)
                    {
                        ...
                        var mf = GetFirstInRange(transferTo.position, mineralFields, resourceRange);
                        var gb = GetFirstInRange(transferTo.position, gasBuildings, resourceRange);
        ...
        public static void Construct(uint unitType)
        {
            ...
            while (true)
            {
                ...
                if (!Units.GasGeysersStructures.Contains(unitType))
                {
